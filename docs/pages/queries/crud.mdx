...
The first and most important part of every Blade query is its type (“query type”).

It represents the operation that is being performed at the time when the query is executed and, therefore, the way in which records are either read or written in the database that is being addressed by the query.

## Adding Records (`add`)

To insert individual records for a given Blade model, the `add` query type can be used:
...
As usual with Blade’s query syntax, you may choose to nest the different parts of the query in any way you like, at any level you like:
...
To obtain all records of a given Blade model, the `get` query type can be used:
...
To update individual records of a given Blade model, the `set` query type can be used:
...
Similarily, multiple records of the same Blade model may be updated like so:
...
To remove individual records of a given Blade model, the `remove` query type can be used:
...
Similarily, multiple records of the same Blade model may be removed like so:
...
To determine the amount of all the records available for a given Blade model, the `count` query type can be used:
