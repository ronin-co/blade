...
You can use sub queries to retrieve records from a related model.
...
use.account((fields) => ({
  with: { handle: 'elaine' },
  including: {
    teams: get.teams.with.member.id(fields.id)
  }
}));
```

The query shown above will return the first Account record whose `handle` field matches `"elaine"` and all team records that are related to that account.
