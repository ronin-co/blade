...
models), you can add a new field of type "Link" to the "Member" model, which could
...
By default, every Link field targets a single record (one-to-one).

In the majority of cases, that usually implies a "child to parent" relationship, meaning
that many child records can target a single parent record, as described [above](#link).

However, there are also cases in which you might want to establish "parent to child"
relationships, which can also be described as "one-to-many". Defining such a
relationship in a Blade model is as simple as setting the `kind` attribute of your
Link field to `many`:
...
export const Account = model({
  ...
  fields: {
    name: string(),
    handle: string(),
    members: link({ target: 'member', kind: 'many' })
  }
});
...
The default value of the `kind` attribute is "one", which defines a "one-to-one"
relationship. Setting it to "many" turns the relationship into "one-to-many".
