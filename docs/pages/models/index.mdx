...
based on your model definitions. Those types are stored in a file named `.blade/types.d.ts` and
...
with type-checking runs on a Git provider, we advice committing the `.blade` directory to your
repository, such that your CI can validate the TypeScript types.

In general, however, the `.blade/output/` directory is ephemeral, meaning it does not need to be
