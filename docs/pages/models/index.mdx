...
The queries defined across your application automatically receive TypeScript types that are based on your model definitions. Those types are stored in a file named `.ronin/types.d.ts` and are updated every time your run `blade apply`.
...
Assuming that your application is located in a Git repository for which an automatic CI process with type-checking runs on a Git provider, we advice committing the `.ronin` directory to your repository, such that your CI can validate the TypeScript types.

In general, however, the `.ronin` directory is ephemeral, meaning it does not need to be committed to your Git repository, since it is automatically re-generated every time you run a migration command (`blade diff` or `blade apply`) in your project.
