...
For example, if the slug of your model is `account`, you could create the following file:

```ts
// triggers/account.ts

import { triggers } from 'blade/'
...
- `options.parentTrigger`: Indicates whether the query was generated by another trigger.
  If it was, the property will contain its model slug and its trigger type.
- `options.headless`: Indicates whether the query is coming from "the outside", meaning
  either from the [client-side of Blade](/hooks#usemutation-client) or the auto-generated
  ...
  stems from the "inside", meaning the server-side.
...
- `options.cookies`: An object containing all cookies of the request that ran the query.
- `options.setCookie`: A function that can be used to add, update, or remove a cookie. It
  takes the same arguments as the cookie setting function offered by
  the [useCookie](/hooks#usecookie) hook.
- `options.context`: A `Map` used for sharing arbitrary values between triggers.
