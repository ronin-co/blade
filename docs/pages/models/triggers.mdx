...
- `options.parentTrigger`: Indicates whether the query was generated by a trigger instead
  of arriving from the outside. If it was, the property will contain its model slug and
  its trigger type.
- `options.headless`: Indicates whether the query is coming from "the outside", meaning
  ...
  [REST API](https://blade.im/api-routes#api-routes). In those cases, you should consider
