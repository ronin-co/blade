...
- `options.waitUntil`: A `Map` that can be used for sharing arbitrary values between triggers.

Additional options are available for [Sink](#Trigger%20Sink) triggers.

## Trigger Context

In order to share arbitrary values between the trigger of a model, you can store them
...
value of the stripped field available to any triggers after the current trigger.
