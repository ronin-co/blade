...
you can pass a `targetRecord` prop with field values that should be used to resolve the
...
<Form model="account" targetRecord={{ email: account.email }}>
  <input type="text" name="password" placeholder="Password" />
  <button type="submit">Submit</button>
</Form>
```
...
As you can see, all that the `targetRecord` prop does is switch the write query from
...
import { Form, FormFields, FormContext } from 'blade/client/components';
...
Note that, if you choose to place the submit button outside `FormFields`, Blade will not be
...
<ParamField path="targetRecord" type="object">
