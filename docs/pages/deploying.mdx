...
Blade will automatically detect Netlify based on the presence of the `NETLIFY`
environment variable (which Netlify provides by default) and adjust its build output
...
When deploying to Netlify, the `.blade/dist/` directory generated by Blade must be manually
selected as the "Publish directory". This will be automated in the future.
...
from the `.blade/dist/` directory, without making use of any other files.
...
COPY --from=build /usr/src/app/.blade/dist/ .blade/dist/
